...
- If an [[arrays|array]] of a specific size is initialized but not all the elements are filled with actual values, the remaining elements will default to **0**
...
A [[structures|structure]] is a collection of **related** data members of a **similar type** under one name.
...
A [[pointers|pointer]] is an **address variable** that is meant for storing the address of data, not the data itself. It's used for **indirectly** accessing the data.
...
- [[Heap]] memory is external to the program, outside of it
...
- It's a C function used to allocate dynamic memory in the [[heap|Heap]]
...
However, sometimes we don't know how much memory we will need until the program is actually running. `malloc` bypasses the [[stack-memory|Stack]] and goes straight to the [[Heap]] to grab a custom-sized chunk of data that we control completely.
#### Allocation
...
A [[references|reference]] is a nickname given to a variable. When declaring, references they **must be initialized**.
